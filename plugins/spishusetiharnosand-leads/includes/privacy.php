...
add_filter(
	...
	function ( $exporters ) {
		$exporters['spishusetiharnosand-leads'] = array(
			'exporter_friendly_name' => __( 'Quote requests', 'spishusetiharnosand-leads' ),
			'callback'               => function ( $email ) {
				$items = array();
				foreach ( npl_leads_by_email( $email ) as $lead ) {
					$data = array( array( 'name' => __( 'Received', 'spishusetiharnosand-leads' ), 'value' => $lead->post_date ) );
					...
					$items[] = array(
						'group_id'    => 'spishusetiharnosand-leads',
						'group_label' => __( 'Quote requests', 'spishusetiharnosand-leads' ),
...
add_filter(
	...
	function ( $erasers ) {
		$erasers['spishusetiharnosand-leads'] = array(
			'eraser_friendly_name' => __( 'Quote requests', 'spishusetiharnosand-leads' ),
			'callback'             => function ( $email ) {
				$removed = false;
				foreach ( npl_leads_by_email( $email ) as $lead ) {
