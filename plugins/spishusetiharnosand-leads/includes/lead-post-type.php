...
add_action(
	...
	function () {
		register_post_type(
			...
			array(
				'labels'          => array(
					'name'          => __( 'Leads', 'spishusetiharnosand-leads' ),
					'singular_name' => __( 'Lead', 'spishusetiharnosand-leads' ),
					'edit_item'     => __( 'Förfrågan', 'spishusetiharnosand-leads' ),
					'search_items'  => __( 'Search leads', 'spishusetiharnosand-leads' ),
					'not_found'     => __( 'Inga förfrågningar än. Skicka formuläret så dyker den upp här.', 'spishusetiharnosand-leads' ),
					'all_items'     => __( 'Alla förfrågningar', 'spishusetiharnosand-leads' ),
				),
...
add_filter(
	...
	function () {
		return array(
			...
			'title'        => __( 'Name', 'spishusetiharnosand-leads' ),
			'npl_phone'    => __( 'Phone', 'spishusetiharnosand-leads' ),
			'npl_service'  => __( 'Needs', 'spishusetiharnosand-leads' ),
			'npl_timeline' => __( 'When', 'spishusetiharnosand-leads' ),
			'npl_zip'      => __( 'ZIP', 'spishusetiharnosand-leads' ),
			'npl_source'   => __( 'Source', 'spishusetiharnosand-leads' ),
			'npl_status'   => __( 'Status', 'spishusetiharnosand-leads' ),
			'date'         => __( 'Received', 'spishusetiharnosand-leads' ),
		);
	}
);
...
function npl_source_summary( $post_id ) {
	...
	return __( 'Direct', 'spishusetiharnosand-leads' );
}
...
add_action(
	...
	function () {
		add_meta_box( 'npl_details', __( 'What they told us', 'spishusetiharnosand-leads' ), 'npl_details_box', 'npl_lead', 'normal', 'high' );
		add_meta_box( 'npl_status', __( 'Status', 'spishusetiharnosand-leads' ), 'npl_status_box', 'npl_lead', 'side', 'high' );
	}
);
...
function npl_details_box( $post ) {
	$rows = array(
		__( 'Name', 'spishusetiharnosand-leads' )     => get_post_meta( $post->ID, 'name', true ),
		__( 'Phone', 'spishusetiharnosand-leads' )    => get_post_meta( $post->ID, 'phone', true ),
		__( 'E-post', 'spishusetiharnosand-leads' )    => get_post_meta( $post->ID, 'email', true ),
		__( 'ZIP', 'spishusetiharnosand-leads' )      => get_post_meta( $post->ID, 'zip', true ),
		__( 'Needs', 'spishusetiharnosand-leads' )    => npl_label( 'service', get_post_meta( $post->ID, 'service', true ) ),
		__( 'When', 'spishusetiharnosand-leads' )     => npl_label( 'timeline', get_post_meta( $post->ID, 'timeline', true ) ),
		__( 'Source', 'spishusetiharnosand-leads' )   => npl_source_summary( $post->ID ),
	);
	...
	$rows[ __( 'Samtycke lämnat', 'spishusetiharnosand-leads' ) ] = get_post_meta( $post->ID, 'consent_at', true ) . ' — "' . get_post_meta( $post->ID, 'consent_text', true ) . '"';
...
function npl_status_box( $post ) {
	...
	echo '<label class="screen-reader-text" for="npl-status">' . esc_html__( 'Status', 'spishusetiharnosand-leads' ) . '</label><select id="npl-status" name="npl_status" style="width:100%">';
