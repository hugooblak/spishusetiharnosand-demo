 ...
 * or photo. They call sp_lead() instead. To build a demo for a real firm, the
 ...
 *
 * With no lead-data.php present the defaults below render the fictional Spishuset I Härnösand.
 *
 ...
 * @return string
 */
function sp_sample_note() {
	return sp_lead( 'ar_riktig' ) ? '' : ' <span class="sp-sample">(platshållare)</span>';
}
 ...
 * @return string File name for sp_lead_img().
 */
function sp_lead_bild( $i ) {
	$bilder = (array) sp_lead( 'bilder', array() );
 ...
 * @return string
 */
function sp_ring_label() {
	$namn = sp_lead( 'kontakt' );
	return $namn ? sprintf( 'Ring %s · %s', $namn, sp_lead( 'telefon' ) ) : 'Ring ' . sp_lead( 'telefon' );
}
